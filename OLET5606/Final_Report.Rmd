...
This project endeavors to explore whether the anatomical and demographic variables contained in this dataset can predict whether an individual is cognitively healthy (CDR = 0) or has some degree of dementia (CDR > 0). INSERT CITATION ABOUT PREDICTING CDR SCORES. This question will be explored using two different types of binary classifiers: logistic regression and linear support vector machine.
...
To move forwar with classification analysis, a binary variable (`has_dementia`) can be constructed from `CDR`:

```{r}
oasis_data <- oasis_data %>%
  dplyr::mutate(has_dementia = CDR > 0, .keep = "unused")
...
```{r}
# Logistic regression
LR_model <- glm(has_dementia ~ ., data = training_data, family = 'binomial')
...
```{r}
RF_model <- randomForest(has_dementia ~ ., data=training_data, ntree=100, mtry=3, importance=TRUE)
```
...
```{r}
# First make in-sample predictions as a baseline comparison
RF_train_pred_prob <- predict(RF_model, newdata = training_data, type = "response")
RF_train_preds <- ifelse(RF_train_pred_prob < 0.5, FALSE, TRUE)

# Then make out-of-sample predictions
RF_test_pred_prob <- predict(RF_model, newdata = testing_data, type = "response")
RF_test_preds <- ifelse(RF_test_pred_prob < 0.5, FALSE, TRUE)
```


## 3.3 Results

Now that the two binary classification models have been applied to the testing data, their respective predictive performances can be compared.

```{r}
testing_data %>%
  dplyr::rename("Real" = "has_dementia") %>%
  dplyr::mutate("LR_train_predicted" = LR_train_preds,
                "RF_train_predicted" = RF_train_preds,
                "LR_test_predicted" = LR_test_preds,
  ...
  dplyr::summarise(LR_test_accuracy = sum(LR_test_predicted == Real)/n(),
                   ...
                   LR_test_accuracy = sum(LR_test_predicted == Real)/n(),
                   RF_test_accuracy = sum(RF_test_predicted == Real)/n())
```
